...
// Jürgen Brauer:
// this is the name of the plugin that we scan for in the Lua script part running in V-REP
// So the name of the .dll file is actually not important.
// You can name your .dll file xyz.dll, even if the plugin name is BubbleRob.
...
struct sBubbleRob
{
    int handle;
    int motorHandles[2];
    int sensorHandleLeft;
    int sensorHandleRight;
...
// Jürgen Brauer:
// Here we define the name of the Lua script function we want to use from the V-REP child
// script.
...
void LUA_CREATE_CALLBACK(SScriptCallBack* cb)
{
    ...
    if (D.readDataFromStack(cb->stackID,inArgs_CREATE,inArgs_CREATE[0],LUA_CREATE_COMMAND))
    {
        ...
        bubbleRob.sensorHandleLeft=inData->at(1).int32Data[0];
        bubbleRob.sensorHandleRight=inData->at(2).int32Data[0];
        bubbleRob.backRelativeVelocities[0]=inData->at(3).floatData[0];
        bubbleRob.backRelativeVelocities[1]=inData->at(3).floatData[1];
...
{ // This is called just once, at the start of V-REP.
    ...
    simAddStatusbarMessage("*******************************\n");
    simAddStatusbarMessage("Welcome to Prof. Dr. Juergen Brauer's Braitenberg vehicle simulation!\n");
    simAddStatusbarMessage("*******************************\n");
    simAddStatusbarMessage("\n\n");
    ...
    if (message==sim_message_eventcallback_modulehandle)
    {
        if ( (customData==NULL)||(std::string("BubbleRob").compare((char*)customData)==0) ) // is the command also meant for BubbleRob?
        {
            ...
            for (unsigned int i=0;i<allBubbleRobs.size();i++)
            {
                ...
                {
                   simAddStatusbarMessage("Two sensors version:");

                   char txt[500];
                   float distanceLeft  = -1.0f;
                   float distanceRight = -1.0f;
                   float point[4];

                   float sensorValueLeft = simReadProximitySensor(allBubbleRobs[i].sensorHandleLeft, point, NULL, NULL);
                   sprintf_s(txt, "\tsensor value left: %.3f", sensorValueLeft);                   
                   simAddStatusbarMessage(txt);
                   sprintf_s(txt, "\tdetected point left: %.3f %.3f %.3f %.3f", point[0], point[1], point[2], point[3]);                   
                   simAddStatusbarMessage(txt);
                   if (sensorValueLeft>0)
                     distanceLeft = point[3];

                   float sensorValueRight = simReadProximitySensor(allBubbleRobs[i].sensorHandleRight, point, NULL, NULL);
                   sprintf_s(txt, "\tsensor value right: %.3f", sensorValueRight);
                   simAddStatusbarMessage(txt);
                   sprintf_s(txt, "\tdetected point right: %.3f %.3f %.3f %.3f", point[0], point[1], point[2], point[3]);
                   simAddStatusbarMessage(txt);
                   if (sensorValueRight>0)
                     distanceRight = point[3];
                      
                   float max_speed = 3.1415f*3.0f;
                   
                   // no distance values
                   if ((distanceLeft == -1) && (distanceRight == -1))
                   {
                      simSetJointTargetVelocity(allBubbleRobs[i].motorHandles[0], max_speed);
                      simSetJointTargetVelocity(allBubbleRobs[i].motorHandles[1], max_speed);
                   }
                   
                   // there is something to the left! decrease right motor speed
                   if (distanceLeft != -1)
                   {                      
                      float val = (0.5f-distanceLeft);
                      val = (val/0.5f)*max_speed;
                      simSetJointTargetVelocity(allBubbleRobs[i].motorHandles[1], max_speed - val);
                      sprintf_s(txt, "\tdecreasing right motor speed by: %.3f", val);
                      simAddStatusbarMessage(txt);
                   }

                   // there is something to the right! decrease left motor speed
                   if (distanceRight != -1)
                   {
                      float rnd = (float)(rand() % 101) / 100.0f; // in [0,1]
                      float val = (0.5f - distanceRight);
                      val = ((val/0.5f)*max_speed)*(2.0f+rnd*5.0f);
                      simSetJointTargetVelocity(allBubbleRobs[i].motorHandles[0], max_speed - val);
                      sprintf_s(txt, "\tdecreasing left motor speed by: %.3f", val);
                      simAddStatusbarMessage(txt);
                   }
                    ...
                    /*
                    // movement mode
                    if (simReadProximitySensor(allBubbleRobs[i].sensorHandleLeft,NULL,NULL,NULL)>0)
